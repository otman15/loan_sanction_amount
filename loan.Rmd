...
## Object :
   Our goal in this analysis is to predict the loan amount that can be sanctioned to customers who have applied for a home loan using the features provided in the dataset (by XYZ Bank). We will do this by building a model capable of generate predictions of the loan amount  to be approved for a client using a training set.
  We will start by taking a look on the dataset and then exploring the different variables to see if there any trends that can help us in the analysis.
  After cleaning the data we will start building and testing our models.
  Finally we will discuss the result obtained by the different models and chose the one that performed the best.
  The evaluation is based on the RMSE - Root Mean Squared Error built under the caret package. $$\mbox{RMSE} = \sqrt{\frac{1}{n}\sum_{t=1}^{n}e_t^2}$$
  \vspace{0.5cm}
...
## Dataset Description
   ...
   The downloaded zip file contains three data sets : "train", "test" and "submission" but we will be using just the train file since the "test" data doesn't have the output variable in it and is used only for competition on kaggle just like the "submission" file.
...
## dealing with nas
Since there is a lot of features with nas, we will  first deal with those na's column by culumn, removing some and replacing the others \
...
There are 340 nas and 338 values less than 0 which doesn't make sense, we will remove the missing outcomes, and replace the others with 0.
```{r echo=FALSE}
na <- sum(is.na(loan_data$`Loan Sanction Amount (USD)`))
paste("The number of Na's ", na)
neg <- sum(loan_data$`Loan Sanction Amount (USD)` < 0, na.rm = TRUE)
paste("The number of values less than '0' is", neg)
```
...
```{r, echo=FALSE}
table(loan_data$Gender) %>% as.data.frame() %>%  knitr::kable()
```
...
The classes low is predominant, we will replace the nas with the dominant class
```{r echo=FALSE}
...
### dependent
There is 14 classes (1 to 14) in the dependent variable, we can see that the class "2" is prevailing and that some classes have insignificant number (classes from 6 to 14), we will replace nas and classes that are almost insignificant in number with the dominant class.
```{r echo=FALSE}
table(loan_data$Dependents) %>% knitr::kable()
```
...
### Has active card
We can see that the three classes of this variable have almost the same number of count, so We will split the number nas equitably between the 3 classes.
```{r echo=FALSE}
table(loan_data$`Has Active Credit Card`) %>% knitr::kable()
```
...
### property location 
As before there is almost the three classes have almost the same count, we will replace the nas by the three classes equitably.
```{r echo=FALSE}
...
\vspace{0.5cm}
 ...
 From the summary of this continuous feature we can that there are big outliers which must affect the mean,  in this case we will replace nas with the median.
```{r echo=FALSE}  
...
paste("the percentage of nagative values is", ng)
```
...
### Property age
```{r echo=FALSE}
summary(loan_data$`Property Age`) 
```
...
start first with continous features. \
\vspace{0.5cm}
 **heatmap of continuous features**

```{r echo=FALSE}
M <- loan_data %>% select(-`Property ID`) %>% select_if(is.numeric) %>% cor()
heatmap(M, scale = "column")
rm(M)
```
We notice that there is strong correlation between some variables and week correlation for others, we will analyze each feature to see if can get some farther insights.

### customer ID :\
We notice that all the customers in this data are unique.
```{r echo=FALSE}
paste("The number of customers is ", length(unique(loan_data$`Customer ID`)))
```
...
```
The distribution of the age is almost stable a part the existence of a pick in the 18 age value with 14% having the age of 18 years, the mean is 40 years and the range is 47 years.
...
```
 We notice a positive correlation between the income and the loan sanction amount, there also many big outliers and null loan amount values, those may affect the results of a model.
...
```
 The distribution is right skewed, this means that most customers have more than the average of loan expenses.
...
```
There is a strong correlation between the loan amount approved and the current loan expenses, this means the bank tend to approve loans for customers who are already honoring their current loans and that customers who have big expenses tend to apply for big loans. \
...
```
 Although there is a correlation between the two variables, it s not strong enough to conclude that the credit score may affect the loan sanction amount, this is because the amount depends more on the amount requsted which may not be correlated with the credit score. But the credit score still could be used to predict if a customer can get a credit or not at all.
...
```
 The distribution contains big outliers and is right skewed.
...
```
We see that the correlation between the two variables is not strong enough to help us make predictions. 

### property price
```{r echo=FALSE, fig.height=4}
summary(loan_data$`Property Price`)
...
```
There is a strong correlation between the loan amount approved and property price, this is obvious since the amount requested depends on the property age.
This correlation is very useful in predicting the loan amount.
...
```
 We can see that the distribution is right skewed and that many customers (28%) didn't manage to get their request approved. \
...
```
 ...
 There is strong correlation between the sanction amount and : "the amount requested, the property price, the approved percentage and current loan expenses. there is also correlation with credit score even if not strong.
...
```
The number of dependents on its own doesn't seem to highly affect the loan sanction amount.
...
loan_data %>% 
  ...
  theme(axis.text.x = element_blank())
```
 The property location doesn't seem to affect the loan sanctio amount.\
...
loan_data %>% mutate(`Co-Applicant` = as.factor(`Co-Applicant`)) %>%
  ...
  theme(axis.text.x = element_blank())
```
We notice that the most approved loans are the ones that had more than one applicant. The requested loan with no co-applicant seems to be rejected in general.\
...
loan_data %>% ggplot(aes(x = Gender, y = `Loan Sanction Amount (USD)`, 
  ...
  theme(axis.text.x = element_blank())
```
The fact of being a male or female doesn't affect the amount of approved loan.\
...
```
From the boxplot we can see that the fact of having a high income stability affect positively the loan amount approved, but the fact of having a low income doesn't affect highly the amount approved.\
...
loan_data %>% 
  ...
  theme(axis.text.x = element_blank())
```
The profession on its own doesn't affect the amount approved.\
...
```
The class "others" are more likely to apply for a home loan and the number of applicant differ depending on the type of employment.
...
loan_data %>% 
  ...
  theme(axis.text.x = element_blank(), legend.key.size = unit(0.3, "cm"))
```
The distribution of the loan approved differ slightly from type of employment to other. We notice that some types have the chance of getting their request rejected more than others. This could help us define which customers are likely to have their loan sanction amount to be null.
...
## Drop features no needed in models building ####
As we don't need the property ID we will drop it for now.  The customer ID is unique  and sufficient to make final submission in the case we wanted to.
```{r echo=FALSE}
...
## Binarize categorical features 
 ...
 We have already cleaned our data, so now we will binarize th e categorical features and scale the continuous ones.
...
```
 ...
 Th binarization of all the categorical data will make easy to work with a variety of models.
...
**After binarisation the data now will looks like**
```{r echo=FALSE}
...
## scale continuous features exept the outcome

The scale function will permit us to center and scale continuous features. This is very imortant since it allows us to normalize the variation in the data.
```{r echo=FALSE}
...
test_y  : the training outcome used for training\
test_x  : the testing features used to get predictions\
test_y  : the testing outcome used for measure the RMSE\
...
## linear regression
 We will try a linear model with lm() under caret:: using only the continuous continuous features.
...
paste("The rmse with lm is", rmse_lm)
...
paste("The rmse with rpart model is", rmse_rpart)
...
### Get results 
After training the model with cross validation we apply it  with the optimal tunes on the test set and get the rmse. We could have tuned the model more but thid woud have taken much time.
```{r echo=FALSE}
 ### prediction 
...
paste("The rmse  with Rondom forest is", rmse_rf)
...
plot(pred_rf, test_y)
```
As we see the predictions now are correlated with the loan amount, but we are still making mistakes especially for around the zero loan amount, can we do better.
...
### Get the best nround using crossvalidation.
```{r echo=FALSE}
### best nround
...
paste(c("The rmse with Xgboost is", rmse_xgboost))

results <- results %>% add_row(method = "xgboost", rmse = rmse_xgboost)
results %>% knitr::kable()
```

We can see now clearly that with extreme gradinat boosting the results are far better than the other models. The rmse is now very small and in the area of home loans is acceptable.

```{r echo=FALSE}
plot(pred_xgboost, test_y)
```

We can see that the predictions now are close to our target, the correlation between predictions and the loan amount is very strong.\
...
With Xgboost model, we managed to reach an RMSE of 777, which is a very good results since the amounts of home loan are much more bigger.
 We still could get better results if we include more features. Also an approach based on making a classification first of which customer could get
